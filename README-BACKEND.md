...
7. **The Campaign scheduler only runs while the server stays up.**
   ...
   campaign's time, it won't send until you start the server again and its
   scheduled time has already passed (it'll fire on the next 30-second check
   after startup, not silently skip it — but don't rely on this for anything
   time-sensitive without a real job queue eventually).
